...
_veh addAction ["Install cargo cover",{
	{(_this select 0) animateSource [_x,0];} forEach ["bedside_left1_fold","bedside_left2_fold","bedside_right1_fold","bedside_right2_fold"];
	(_this select 0) animateSource ["cargo_cover_hide",0];
},[],1.5,false,true,"","(alive _target) && (vehicle _this == _this) && !(isEngineOn _target) && (_target animationSourcePhase 'cargo_cover_hide')==1",5,false,"cover_action"];
